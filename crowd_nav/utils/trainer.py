...
class RglACTrainer(object):
    ...
    def optimize_epoch(self, num_epochs):
        ...
        for epoch in range(num_epochs):
            ...
            for data in self.data_loader:
                ...
                loss.backward()
                self.optimizer.step()
                value_loss = value_loss + loss1.data.item()
                policy_loss = policy_loss + loss2.data.item()
            logging.debug('{}-th epoch ends'.format(epoch))
    ...
    def optimize_batch(self, num_batches, episode=None):
        ...
        for data in self.data_loader:
            ...
            vx_dist = Beta(alpha_beta_1[:, 0], alpha_beta_1[:, 1])
            vy_dist = Beta(alpha_beta_2[:, 0], alpha_beta_2[:, 1])
            action_log_probs = vx_dist.log_prob(actions[:, 0]).unsqueeze(1) + vy_dist.log_prob(actions[:, 1]).unsqueeze(
                1)
